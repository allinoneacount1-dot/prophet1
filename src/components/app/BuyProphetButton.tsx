...
export function BuyProphetButton({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  const sz =
    size === "sm" ? "h-9 px-3 text-xs" : size === "lg" ? "h-14 px-7 text-base" : "h-11 px-5 text-sm";
  return (
    <button
      onClick={() =>
