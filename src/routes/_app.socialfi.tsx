...
export const Route = createFileRoute("/_app/socialfi")({
  head: () => ({ meta: [{ title: "SocialFi — ProphetSol" }] }),
  component: SocialFi,
});
...
function SocialFi() {
  ...
  return (
    <>
      <PageHeader eyebrow="SocialFi" title="Community · Growth · Reputation" description="Earn XP, climb ranks, collect badges, and invite friends to grow the Prophet network." />

      <div className="grid gap-4 md:grid-cols-4">
        <StatCard label="Your Rank" value="#384" delta={{ value: "+12", positive: true }} icon={<Crown className="h-4 w-4" />} />
        ...
        <StatCard label="Bonus Earned" value="2,140 $PROPHET" delta={{ value: "+82", positive: true }} />
      </div>

      <div className="mt-8 grid gap-4 lg:grid-cols-3">
        <GlassCard className="lg:col-span-2">
          <SectionTitle icon={<Crown className="h-4 w-4" />} title="Global Leaderboard" hint="Updated every block" />
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-left text-[11px] uppercase tracking-widest text-muted-foreground">
                <tr><th className="py-2">#</th><th>User</th><th>XP</th><th>Badge</th></tr>
              </thead>
              <tbody>
                {LEADERS.map((l) => (
                  <tr key={l.rank} className="border-t border-border">
                    ...
                    <td><span className="rounded-full bg-[color:var(--chain)]/15 px-2 py-0.5 text-[11px] text-[color:var(--chain)]">{l.badge}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <GlassCard glow>
          ...
          <div className="rounded-xl border border-border bg-surface-1/40 p-4">
            ...
            <div className="mt-1 flex items-center justify-between">
              ...
              <button onClick={() => { navigator.clipboard?.writeText(code); toast.success("Code copied"); }} className="grid h-8 w-8 place-items-center rounded-lg border border-border hover:chain-glow">
                <Copy className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">Earn 10% of each invitee's first-month protocol fees and 250 XP per signup. {address && <span className="block mt-1 font-mono">{shortAddr(address)}</span>}</p>
          <button onClick={() => toast.success("Share link copied")} className="mt-4 h-10 w-full rounded-lg bg-[linear-gradient(135deg,var(--prophet),var(--chain))] text-xs font-semibold text-primary-foreground">Share invite</button>
        </GlassCard>
      </div>

      <div className="mt-8">
        <SectionTitle icon={<Award className="h-4 w-4" />} title="Achievement Badges" hint="Earned through activity" />
