  ...
  return (
    <>
      <PageHeader
        ...
        title="DePIN · Decentralized Physical Network"
        description="Real-time map of physical infrastructure nodes powering Prophet's compute, storage, and data layers."
      />
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-3">
        <GlassCard className="lg:col-span-2">
          ...
          <div className="relative aspect-[2/1] overflow-hidden rounded-xl border border-border bg-[radial-gradient(ellipse_at_center,oklch(0.78_0.18_285/0.1),transparent_60%)] grid-bg">
            {NODES.map((n, i) => (
              <span
                key={i}
                className="absolute h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[color:var(--chain)] chain-glow"
                style={{
                  left: n.x,
                  top: n.y,
                  animation: `prophet-pulse ${2 + (i % 3)}s ease-in-out infinite`,
                }}
              />
            ))}
            <div className="absolute bottom-3 left-3 rounded-md bg-background/70 px-2 py-1 text-[10px] text-muted-foreground backdrop-blur">
              Asia-Pacific cluster · streaming
            </div>
          </div>
          <div className="mt-4">
        ...
        <GlassCard glow>
          <SectionTitle
            ...
            hint="Estimate node ROI"
          />
          <label className="text-xs text-muted-foreground">Location</label>
          <input
            value={city}
            onChange={(e) => setCity(e.target.value)}
            className="mt-1 h-10 w-full rounded-lg border border-border bg-surface-1/60 px-3 text-sm outline-none focus:border-[color:var(--chain)]"
          />
          <label className="mt-3 block text-xs text-muted-foreground">
            Daily uptime · {hours}h
          </label>
          <input
            type="range"
            min={1}
            ...
            className="mt-1 w-full accent-[color:var(--chain)]"
          />
      ...
      <div className="mt-8">
        <SectionTitle
          ...
          hint="Available hardware kits"
        />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            {
              name: "Prophet Mini Node",
              req: "8GB RAM · 100GB SSD",
              rev: "$32 / mo",
              region: "Global",
            },
            {
              name: "Prophet Edge Node",
              req: "16GB RAM · 1TB NVMe",
              rev: "$108 / mo",
              region: "EU · US",
            },
            {
              name: "Prophet Vault Node",
              req: "32GB RAM · 4TB · GPU",
              rev: "$340 / mo",
              region: "Limited",
            },
          ].map((n) => (
            <GlassCard key={n.name} glow>
              ...
              <button className="mt-3 h-10 w-full rounded-lg border border-[color:var(--chain)]/40 bg-[color:var(--chain)]/10 text-xs text-[color:var(--chain)]">
                Deploy
              </button>
            </GlassCard>
          ))}
