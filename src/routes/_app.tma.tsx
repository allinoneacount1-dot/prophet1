...
export const Route = createFileRoute("/_app/tma")({
  head: () => ({ meta: [{ title: "Telegram Mini App — Prophet" }] }),
  component: TMA,
});
...
function TMA() {
  return (
    <>
      <PageHeader
        eyebrow="TMA Preview Hub"
        title="ProphetSol · Telegram Mini App"
        description="Designed for 800M+ Telegram users. Connect, swap, and stake from inside a chat."
      />

      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          label="TMA Users (beta)"
          value="184K"
          delta={{ value: "+8.4K / day", positive: true }}
          icon={<Users className="h-4 w-4" />}
        />
        <StatCard
          label="One-Click Connects"
          value="142,000"
          ...
          <div className="rounded-[40px] border border-border bg-surface-1/60 p-3 shadow-[0_30px_80px_-20px_oklch(0_0_0/0.6)]">
            <div className="overflow-hidden rounded-[30px] border border-border bg-background">
              <div className="flex items-center justify-between bg-surface-2 px-4 py-3 text-xs">
                ...
                <span className="font-semibold">@ProphetBot</span>
              ...
              <div className="space-y-3 p-4">
                ...
                <div className="rounded-2xl border border-border bg-surface-1/60 p-3">
                  ...
                  <div className="mt-1 text-2xl font-semibold text-[color:var(--chain)]">
                    $1,284.20
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-[10px]">
                  {["Swap", "Stake", "Invite"].map((b) => (
                    <button
                      key={b}
                      className="rounded-xl border border-border bg-surface-1/60 p-3 font-semibold"
                    >
                      {b}
                    </button>
                  ))}
        ...
        <div className="space-y-4">
          <GlassCard>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Provision a Smart Account & gasless transactions inside the chat — no popups, no
              extensions.
            </p>
          </GlassCard>
          <GlassCard>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Native invite tracking, viral coefficients, and tier rewards.
            </p>
          </GlassCard>
          <GlassCard>
            <h3 className="text-base font-semibold">Mobile-first dashboard</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              Portfolio, DeAI signals, staking, and swaps — optimized for thumbs.
            </p>
          </GlassCard>
        </div>
      </div>
