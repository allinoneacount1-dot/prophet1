  ...
  return (
    <>
      <PageHeader
        ...
        description="Cross-chain staking with passive income claim, lock-up boosts, and a built-in APY calculator."
      />
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          value={`${earned.toFixed(2)} $PROPHET`}
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-3">
              ...
              <tbody>
                {POOLS.map((p) => (
                  <tr key={p.name} className="border-t border-border">
                    ...
                    <td>
                      <button
                        onClick={() => toast.success(`Stake flow opened: ${p.name}`)}
                        className="rounded-lg border border-[color:var(--chain)]/40 bg-[color:var(--chain)]/10 px-3 py-1 text-xs text-[color:var(--chain)]"
                      >
                        Stake
                      </button>
                    </td>
        ...
        <GlassCard glow>
          ...
          <div className="mt-4 rounded-xl border border-[color:var(--chain)]/30 bg-[color:var(--chain)]/10 p-4 text-center">
            ...
            <div className="mt-1 text-3xl font-semibold text-[color:var(--chain)]">
              {reward.toFixed(2)}
            </div>
            <div className="text-xs text-muted-foreground">
              $PROPHET in {days} days · {apy}% APY
            </div>
          </div>
          <button
            onClick={() =>
              toast.success("Staked", { description: `${amount} $PROPHET for ${days}d` })
            }
            ...
            <tbody>
              {[
                ...
                <tr key={row[0]} className="border-t border-border">
                  {row.map((c, i) => (
                    <td key={i} className={`py-2 ${i === 0 ? "" : "text-muted-foreground"}`}>
                      {c}
                    </td>
                  ))}
                </tr>
